...
define(
  ...
  RecursiveBacktrackingSolver.prototype.getSolutionIter = function* (csp) {
    yield* this.recursiveSolveIter({}, csp.variables, csp.constraints);
  }

  RecursiveBacktrackingSolver.prototype.recursiveSolveIter = function*(assignments, variables, constraints) {
    if (Object.keys(assignments).length === Object.keys(variables).length) {
        yield util.hashcopy(assignments);
        return;
    }
    ...
    function checkAssignment(nextVar, val) {
        ...
        for (let c in constraints) {
            let args = [];
            let valid = true;
            // Build the argument list for this constraint
            for (let k in constraints[c].variables) {
                let fp = constraints[c].variables[k];
            ...
            if (valid && !constraints[c].fn.apply(null, args)) {
    ...
    // Try the values in the domain
    for (let j in nextVar.domain) {
        let val = nextVar.domain[j];
        if (checkAssignment(nextVar, val)) {
            assignments[nextVar.name] = val;
            yield* this.recursiveSolveIter(assignments, variables, constraints);
            delete assignments[nextVar.name];
        }
    }
}
